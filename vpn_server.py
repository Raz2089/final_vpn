...
def handle_client(client_sock , available_port):
    while True:
        print("connection map len",len(connection_map))
        ...
        print("recieved packet from my client packet goes to " , packet.dst_addr , packet.dst_port , "this is the data" , packet.payload)
        ...
        while True:
            with packets_to_web_lock:
                if packets_to_send_to_web:
                    ...
                    w.send(packet)
                    print("sending packet from my server packet goes to ", packet.dst_addr, packet.dst_port,
                          "this is the data ", packet.payload)
...
def sniff_responses_from_web(): #this function also send the packets back to the client
    ...
    filter_str = f'inbound and (tcp or udp) and ip.SrcAddr == 10.10.10.165'
    seen_packets = set()
        ...
        while True:
            ...
            with map_lock:
                ...
                if entry:
                    ...
                    print("recieved packet from web" , packet_from_web.dst_addr , packet_from_web.dst_port , packet_from_web.payload ,  "sending it back to client")
